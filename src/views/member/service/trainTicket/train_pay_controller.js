...
export default {
	...
	methods: {
		...
		//获取数据
		getData() {
			$http.get('plugin.train.Frontend.Modules.Order.Controllers.goods-buy', this.para, "生成中...").then((response) => {
				if (response.result == 1) {
					this.goodsInfo = response.data;
					this.setDataView(response.data);
				} else {
		...
		submit() {
			...
			$http.get('plugin.train.Frontend.Modules.Order.Controllers.create', this.para, "提交中...").then(function (response) {
	...
	watch: {
		getLangState(val) {
			if (val) {
				this.language = JSON.parse(sessionStorage.languageService).trainPay;
			} else {
				this.language = this.$store.state.service.languageService.trainPay;
			}
		},
	},
