    ...
    methods:{
		...
		initLang(){
			if(sessionStorage.languageService){
				this.language=JSON.parse(sessionStorage.languageService).trainTicketOrder;
			}else{
				this.language=this.$store.state.service.languageService.trainTicketOrder;
			}
		},
		jump(e){
			let type=e.currentTarget.getAttribute("data-index");
			if(type==1){
				this.$router.push(this.fun.getUrl('trainOrderDetail'));
			}else if(type==2){
				//去付款操作
			}else if(type==3){
				MessageBox({
