...
export default {
	...
	methods: {
		...
		getData() {
			...
			$http.get('plugin.phone.Frontend.Modules.Order.Controllers.goods-buy', that.para, "生成中...").then(function (response) {
		...
		submit() {
			...
			$http.get('plugin.phone.Frontend.Modules.Order.Controllers.create', json, "提交中...").then(function (response) {
	...
	watch: {
		getLangState(val) {
			if (val) {
				this.language = JSON.parse(sessionStorage.languageService).broadband;
			} else {
