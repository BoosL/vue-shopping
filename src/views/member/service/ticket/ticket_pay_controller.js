...
export default {
	...
	methods: {
		...
		//获取数据
		getData() {
			$http.get('plugin.air.Frontend.Modules.Order.Controllers.goods-buy', this.para, "生成中...").then((response) => {
		...
		submit() {
			...
			$http.get('plugin.air.Frontend.Modules.Order.Controllers.create', this.para, "提交中...").then(function (response) {
	...
	watch: {
		getLangState(val) {
			if (val) {
				this.language = JSON.parse(sessionStorage.languageService).ticketPay;
			} else {
				this.language = this.$store.state.service.languageService.ticketPay;
			}
		},
	},
