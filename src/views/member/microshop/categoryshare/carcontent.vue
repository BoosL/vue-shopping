...
<div id="carcontent">
	...
	<div class="sote" v-if='list.data'>
		<h3 class="" v-if='item.name'><router-link :to="fun.getUrl('micro_shop_share_carelist',{id:item.id})">{{item.name}}</router-link></h3>

		<ul v-if="item.has_many_children.length>0">
			<li v-for="catshop in item.has_many_children">
				<router-link :to="fun.getUrl('micro_shop_share_carelist',{id:catshop.id})">
