...
export default {
    ...
    methods: {
        ...
        getData(_apply_status) {
            let that = this;
            $http.get("plugin.micro.frontend.controllers.MicroShopBonusLog.list.apply", { "apply_status": _apply_status, "time_type": this.time_type },"加载中").then(function (response) {
                console.log("分红列表");
        ...
        //去详情
        toInfo(id){
             this.$router.push(this.fun.getUrl('microShop_shareDetail', { orderID:id }));
        }
    },
    components: { cTitle }
