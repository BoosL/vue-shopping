    ...
    <div id="orderList">
        ...
        <el-checkbox-group v-model="checkList"
            ...
            <div class="shop"
                ...
                <router-link :to='fun.getUrl(detailUrl,{order_id:order.id, orderType:orderType})'
                             v-for=" good in order.has_many_order_goods">
                    <div class="goods">
                        ...
                        <div class="price">
                            <div class="money"> ￥{{good.goods_price}}</div>
                            <div> ×{{good.total}}</div>
                        </div>
                        </div>
...
export default {
  ...
  methods: {
    ...
    toPay(item, btn) {
      ...
      this.$router.push(
        this.fun.getUrl("orderpay", { status: 2, order_ids: item.id })
      );
    },
    //评价订单
    comment(item, btn) {
    ...
    confirmOrder(item, btn) {
      MessageBox({
        ...
        if (action == "confirm") {
          ...
          //调用接口通知服务器收到货物请求
          var that = this;
          $http.get(btn.api, { order_id: item.id }, "操作中...").then(
            function(response) {
    ...
    toRefundDetail(item, btn) {
      this.$router.push(
        this.fun.getUrl("aftersales", { refund_id: item.refund_id })
      );
    },
    //退款
    toRefund(item, btn) {
      //refund
      this.$router.push(
        this.fun.getUrl("refund", { order_id: item.id, type: 1 })
      );
    },
